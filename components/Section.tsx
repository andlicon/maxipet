...
import { TO_TOP, TO_LEFT, TO_RIGHT } from '../constant/components';
...
const Section = ({
  ...
  const getAdditionalElement = (classes: string) => {
    if (additional.contentType === 'image') {
      return (
        <img
          src={additional.content}
          className={`section__img self-start ${classes}`}
          alt=""
        />
      );
    }
      ...
      <div>
        ...
        <div
          className={`flex flex-col items-center md:flex-row${
            additional?.placement === TO_TOP ? ' flex-wrap' : ''
          }`}
        >
          {additional ? displayAdditional() : null}
